  ...
  if (q == n) print(row, col);
    ...
    for (int i = 0; i < n; i++) {
      if (not col[i] and not diag_up[i+q] and not diag_down[q-i+n-1]) {
        ...
        backtracking(q+1, row, col, diag_up, diag_down);
...
int main() {
  ...
  backtracking(0, row, col, diag_up, diag_down);
}
